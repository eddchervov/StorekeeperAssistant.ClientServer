...
namespace StorekeeperAssistant.Api.Utils.Implementation
{
    public static class ApiUtils
    {
        ...
        public static async Task<T> ExecuteGetAsync<T>(string url, string accessToken, int? timeout = null)
        {
        ...
        public static async Task<T> ExecuteDeleteAsync<T>(string url, int? timeout = null)
        {
            using (var client = new HttpClient())
            {
                if (timeout.HasValue)
                ...
                var httpResponse = await client.DeleteAsync(new Uri(url));

                return await ProcessResponseMessageAsync<T>(httpResponse, url);
            }
        }
        ...
        public static async Task<T> ExecutePutAsync<T, U>(string url, U data, int? timeout = null)
        {
            var parameters = JsonConvert.SerializeObject(data, JsonSerializerSettings);

            return await ExecutePutAsync<T>(url, parameters, timeout);
        }

        public static async Task<T> ExecutePutAsync<T>(string url, string parameters, int? timeout = null)
        {
            using (var client = new HttpClient())
            {
                if (timeout.HasValue)
                ...
                var httpResponse = client.PutAsync(url, content).Result;
        ...
        private static async Task<T> ProcessResponseMessageAsync<T>(HttpResponseMessage responseMessage, string url)
        {
            if (responseMessage.StatusCode == HttpStatusCode.NoContent)
            {
                return default(T);
            ...
            if (responseMessage.StatusCode != HttpStatusCode.OK)
                throw new WebException($"Request({url}) fault with code = {responseMessage.StatusCode}");
